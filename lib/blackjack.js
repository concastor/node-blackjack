...
BlackjackGame.prototype.getResult = function () {
	...
	if (this.playerHand.isBust()) {
		if (dealerScore > aiScore) {
			...
			if (this.AI_hand.getCards.length < this.dealerHand.getCards.length) {
				...
				return "you bust, AI and Dealer Tie"
			}
		}
		//dealer is bust compare
	...
	} else if (this.AI_hand.isBust()) {
		if (dealerScore > playerScore) {
			return "Dealer Win"
		} else if (dealerScore < playerScore) {
			return "Win"
		} else if (playerScore == dealerScore) {
			if (this.playerHand.getCards.length < this.dealerHand.getCards.length) {
				return "Dealer Win"
			} else if (
				this.playerHand.getCards.length > this.dealerHand.getCards.length
			) {
				return "Win"
			} else {
				return "Player and Dealer Tie"
			}
		}
	}
...
BlackjackGame.prototype.stand = function () {
	if (this.isGameInProgress()) {
		...
		while (!this.aiStay) {
			this.makeAiMove()
			console.log("ai sya", this.aiStay)
		}
